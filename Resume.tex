\documentclass[a4paper, 11pt]{article}

\usepackage[english]{babel}
\usepackage[utf8x]{inputenc}
\usepackage[protrusion=true,expansion=true]{microtype}
\usepackage{amsmath,amsfonts,amsthm}     % Math packages
\usepackage{graphicx}                    % Enable pdflatex
\usepackage[svgnames]{xcolor}            % Colors by their 'svgnames'
\usepackage{geometry}
\geometry{
	a4paper,
	total={170mm,257mm},
	left=20mm,
	top=20mm,
	}
\usepackage{hyperref}

%\frenchspacing              % Better looking spacings after periods
\pagestyle{empty}           % No pagenumbers/headers/footers

\usepackage{sectsty}

\sectionfont{%                      % Change font of \section command
	\usefont{OT1}{phv}{b}{n}%       % bch-b-n: CharterBT-Bold font
	\sectionrule{0pt}{0pt}{-3pt}{1pt}}

\newlength{\spacebox}
\settowidth{\spacebox}{88888888}            % Box to align text
\newcommand{\sepspace}{\vspace*{0.3em}}       % Vertical space macro

\newcommand{\MyName}[1]{ % Name
        \noindent
		\Huge \usefont{OT1}{phv}{b}{n} #1
		\par \normalsize \normalfont}

\newcommand{\MySlogan}[1]{ % Slogan (optional)
		\noindent
		\large \usefont{OT1}{phv}{m}{n}\textit{#1}
		\par \normalsize \normalfont}

\newcommand{\NewPart}[1]{\section*{\uppercase{#1}}}

\newcommand{\PersonalEntry}[2]{
		\noindent\hangindent=2em\hangafter=0 % Indentation
		\parbox{\spacebox}{        % Box to align text
		\hfill\textit{#1}}		       % Entry name (birth, address, etc.)
		\hfill\hspace{1.2em} #2 \par}    % Entry value


\newcommand{\SummaryEntry}[1]{
		\noindent\hangindent=2em\hangafter=0
		\noindent #1
		\normalsize \par}

\newcommand{\SkillsEntry}[2]{      % Same as \PersonalEntry
		\noindent\hangindent=2em\hangafter=0 % Indentation
		\parbox{\spacebox}{        % Box to align text
		\textit{#1}}			   % Entry name (birth, address, etc.)
		\hspace{1.2em} #2 \par}    % Entry value

\newcommand{\EducationEntry}[4]{
		\noindent \textbf{#1} \hfill      % Study
		\colorbox{White}{%
			\parbox{5cm}{%
			\hfill\color{Black}#2}} \par  % Duration
		\noindent \textit{#3} \par        % School
		\noindent\hangindent=2em\hangafter=0 \small #4 % Description
		\normalsize \par}

\newcommand{\WorkEntry}[4]{				  % Same as \EducationEntry
		\noindent \textbf{#1} \hfill      % Jobname
		\noindent\colorbox{Black}{\color{White}#2} \par  % Duration
		\noindent \textit{#3} \par              % Company
		\noindent\hangindent=2em\hangafter=0 \small #4 % Description
		\normalsize \par}

\newcommand{\PrjEntry}[4]{				  % Same as \EducationEntry
		\noindent \textbf{#1} \hfill      % Jobname
		\noindent\colorbox{Black}{\color{White}#2} \par  % Duration
		\noindent \textit{#3} \par              % Company
		\noindent\hangindent=2em\hangafter=0 \small #4 % Description
		\normalsize \par}

%%% Begin Document
%%% ------------------------------------------------------------
\begin{document}

\parbox{0.35\textwidth}{
	\MyName{Ben Liu}
	\MySlogan{Software Engineer}
}
\parbox{0.6\textwidth}{
%%% Personal details
%%% ------------------------------------------------------------

\PersonalEntry{}{\href{https://www.linkedin.com/in/ben-liu}{\texttt{linkedin.com/in/ben-liu}}}
\PersonalEntry{}{\href{https://github.com/ben196888}{\texttt{github.com/ben196888}}}
\PersonalEntry{}{\href{tel:+66-825-811-373}{\texttt{(+66) 825 811 373}}}
% \PersonalEntry{}{\href{tel:+61-413-188-912}{\texttt{(+61) 413 188 912}}}
% \PersonalEntry{}{\href{tel:+886-921-960-027}{\texttt{(+886) 921 960 027}}}
\PersonalEntry{}{\href{mailto:ben196888@gmail.com}{\texttt{ben196888@gmail.com}}}
}
\sepspace

%%% Summary
%%% ------------------------------------------------------------
\NewPart{Summary}{}

\SummaryEntry{%
Ben Liu is focusing on full-stack web apps developing these 5 years.
He worked in JavaScript with ReactJS for 3.5 years and NodeJS for 2 years.
He is always passionate about new technology and likes to learn new ideas from developer events and open source projects.
He is not only an avid problem solver but also a good autodidact who is keen on more than one solution.
}

%%% Skills
%%% ------------------------------------------------------------
\NewPart{Skills}{}

\SkillsEntry{Skills}    {TypeScript, JavaScript, Golang, Docker, C\#}
\SkillsEntry{Libraries} {ReactJS, Redux, RxJS, NodeJS}
\SkillsEntry{Databases} {Firebase, Mongo, PostgreSQL, MSSQL}
\SkillsEntry{Tools}     {Elasticsearch, Kafka}

%%% Work experience
%%% ------------------------------------------------------------
\NewPart{Work Experience}{}

\WorkEntry{Agoda, Bangkok, Thailand}{Sep. 2019 - Present}{Full-Stack Software Engineer}{
\begin{itemize}
	% Campaign system
	\item {Design the dynamic }
	\item {Led 5 Devs and 1}
	% Member based coupon
	% Mini Program
	\item {Introduce new client widgets for Agoda to win China market.}
\end{itemize}
}

\WorkEntry{Predictive Hire, Melbourne, Australia}{Oct. 2018 - Jun. 2019}{Software Engineer}{
\begin{itemize}
	% hudson
	% fidelity, frank group, quantium, qantas, regis, employsure, woolworth, jetstar, medibank, aldi, officework
	% \item {Designed and built a chatbot for 6000 applicants that brought a half-million AUD investment.}
	% Platform + Predictor Py + CX
	\item {Reduced 600\% CI/CD run time by decoupling micro-services in same integration test rate.}
	% Page Up integration - ALDI
	\item {Led 4 Devs and 1 DevOps to start migrating monolithic API services to micro-services.}
	% Reporting
	\item {Guided 2 data specialists to process and ship 1.5 GB data to report customers every month.}
\end{itemize}
}

\sepspace

\WorkEntry{Tabcorp, Melbourne, Australia}{Jan. 2017 - Sep. 2018}{Developer}{
\begin{itemize}
	% Transaction Geo Location recording service - integrate with Brisbane Tatt team
	\item {Led 3 Devs and 1 DevOps to build an API service for 5000 transactions per min in 4 weeks.}
	% Rubix integrate operator feedback system with Salesforce SSO
	\item {Solved 3rd party API issues across the Ops team and Devs team on production in one day.}
	% Replace chorme headless with JSDOM for SVG unit tests
	\item {Reduced DOM unit tests 3500\% execution time by replacing headless chrome with JSDOM.}
	% Rubix
	\item {Increased unit tests coverage rate from 75\% to higher than 95\% by adding 100+ test cases.}
	% Rubix performance issue - sync function call -> async function call
	\item {Resolved the browser crashed issue by throttling when receiving 150+/s data stream.}
	% Rubix performance issue - queueing the transaction before send them to Ag Table
	\item {Unblocked bottleneck to render 500\% more transactions per second by queueing the stream.}
\end{itemize}
}

\sepspace

\WorkEntry{HopebayTech, Taipei, Taiwan}{Feb. 2014 - Oct. 2016}{Software Engineer}{
\begin{itemize}
	% Human resource web app with google scripts and google docs
	\item {Designed and developed an internal human resource system to replace the paperworks of 100+ employees.}
	% CC front end, CC back end, Monioring front end, Tera OS, Tera Storage
	\item {Created 5 continuous integration pipelines for 5 web apps and 2 cross-compiling build systems.}
\end{itemize}
}

%%% Education
%%% ------------------------------------------------------------
\NewPart{Education}{}

\EducationEntry{National Taiwan University, Taipei, Taiwan}{Jul. 2009 - Feb. 2014}
{Bachelor of Science degree in Mathematics}{}

%%% Projects
%%% ------------------------------------------------------------
% \NewPart{Project}{}
% \PrjEntry{}{}{}{}
%\PrjEntry{}{}{}{}
%\PrjEntry{}{}{}{}

%%% References
%%% ------------------------------------------------------------
%\NewPart{References}{}
%Available upon request
\end{document}
